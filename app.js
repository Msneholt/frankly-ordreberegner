    ...
    <div class="workspace">
      ...
      <aside class="card result-card" aria-live="polite">
        ...
        <div class="result-footer">
          ...
          <div class="actions">
            <button id="copyButton" class="secondary" type="button">Kopiér tilbud</button>
  ...
  <script>
    (function(){
      ...
      function getState(){
        ...
        var offerNumber=document.getElementById("offerNumber").value.trim();
      ...
      function renderCleanQuote(state,selected,deliveryText,totalText,totalVatText){
        ...
        document.getElementById("cleanQuoteTotals").innerHTML='<div class="clean-quote-total"><span>Produkter</span><strong>'+money.format(state.pricedSubtotal)+' DKK</strong></div>'+deliveryRows(state,"clean-quote-total",deliveryText)+'<div class="clean-quote-total grand"><span>'+(state.oneTime?"Total for bestillingen ekskl. moms":"Total pr. levering ekskl. moms")+'</span><strong>'+totalText+'</strong></div><div class="clean-quote-total"><span>Moms 25 %</span><strong>'+(state.priceReady?money.format(state.totalDelivery*.25)+" DKK":"—")+'</strong></div><div class="clean-quote-total"><span>Total inkl. moms</span><strong>'+totalVatText+'</strong></div>';
        var employeeMetricsReady=state.priceReady&&state.employeesRelevant&&!state.oneTime;
      ...
      function summaryText(){
        ...
        lines.push("Virksomhed: "+company,"Kontaktperson: "+contact,"Postnummer: "+postalCode,"Antal medarbejdere: "+(state.employeesRelevant?integers.format(state.employees):"Ikke relevant"),"Levering: "+cadenceText(state.cadence,state.oneTime),"Prisgruppe: "+state.tier.name,"",state.oneTime?"Produkter i bestillingen:":"Produkter pr. levering:");
        var selected=products.filter(function(product){return quantity(product)>0;});
      ...
      document.getElementById("copyButton").addEventListener("click",function(){
        ...
        if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(text).then(function(){showToast("Oversigten er kopieret");}).catch(function(){fallbackCopy(text);});}
        else fallbackCopy(text);
      });
      function fallbackCopy(text){var area=document.createElement("textarea");area.value=text;area.setAttribute("readonly","");area.style.position="fixed";area.style.opacity="0";document.body.appendChild(area);area.select();var copied=false;try{copied=document.execCommand("copy");}catch(error){copied=false;}area.remove();showToast(copied?"Oversigten er kopieret":"Kunne ikke kopiere automatisk");}
      document.getElementById("saveButton").addEventListener("click",function(){
        render();var state=getState();var quote=document.getElementById("cleanQuote").cloneNode(true);quote.removeAttribute("aria-hidden");quote.style.display="block";var styles=document.querySelector("style").textContent;var content='<!doctype html><html lang="da"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Frankly pristilbud</title><style>'+styles+'</style></head><body>'+quote.outerHTML+'</body></html>';var blob=new Blob([content],{type:"text/html;charset=utf-8"});var link=document.createElement("a");var company=state.company||"kunde";var numberPart=state.offerNumber?"-"+state.offerNumber:"";link.href=URL.createObjectURL(blob);link.download=("Frankly-pristilbud-"+company+numberPart).replace(/[^a-z0-9æøå_-]+/gi,"-")+".html";link.click();URL.revokeObjectURL(link.href);showToast("Det rene pristilbud er gemt");
      });
